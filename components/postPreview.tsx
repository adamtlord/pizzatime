...
export default function PostPreview({ post }) {
  const { title, slug, preview, body, image } = post.fields;
  ...
  return (
    <div className="flex flex-col rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
      ...
      <div className="flex-1 p-6 flex flex-col justify-between">
        ...
        <Link href={`/blog/${slug}`} passHref>
          <a className='text-red-700 mt-6 font-bold flex items-center'>Read all about it <ArrowNarrowRightIcon className="h-5 w-5 ml-2" /></a>
        </Link>
      </div>
    </div>
  )
